...
every deliverable, the estimator was evaluated over $N=1000$ Monte Carlo trials and
compared against the known ground-truth trajectory.

The least-squares deliverables solve for the full trajectory in batch form, while the
Kalman filter deliverables estimate the state incrementally over time. In all cases, the
...
For Deliverable 1, the batch least-squares problem was built using only the initial pose
constraint and the odometry displacement equations between consecutive robot position
states. Since no landmark observations are included, the estimate is driven entirely by
...
The mean absolute error grows with time because the robot never receives an external
positional correction. Each noisy odometry measurement contributes a small amount of
drift, and the accumulated effect becomes larger as the robot moves farther down the
hallway.

\section{Deliverable 2: Batch Least Squares with Odometry and Landmark Sensing}
For Deliverable 2, the batch least-squares formulation was extended to include signed
landmark range measurements of the form $l_j - x_k$. This keeps the measurement model
linear while allowing the batch solve to estimate both the robot trajectory and the
three landmark positions.
...
The robot's observation of landmarks reduces the growth of error. When landmarks are
observed, the batch system gains additional constraints that tie the robot's pose states
to fixed features in the hallway. This limits the amount of odometry drift that can
accumulate and keeps the mean absolute error lower than in the odometry-only case.

\section{Deliverable 3: Kalman Filter with Odometry Only}
For Deliverable 3, a linear discrete-time Kalman filter was implemented with state
vector
\[
...
was chosen according to the assignment, with only $Q(1,1)$ nonzero.
...
Compared with Deliverable 1, the odometry-only Kalman filter shows the same overall
behavior: error grows over time because the estimator relies only on noisy motion
information and receives no landmark-based correction. The main additional benefit of
the Kalman filter is that it also predicts its own uncertainty through $P_k$. In the
plot, the covariance-based position standard deviation increases with time in a manner
that is consistent with the rising empirical error.

\section{Deliverable 4: Kalman Filter with Odometry and Landmark Sensing}
For Deliverable 4, the Kalman filter was extended to use both odometry and landmark
measurements. The measurement matrix $H_k$ and noise covariance $R_k$ were allowed to
change with time depending on which landmarks were within sensor range at each time
step.
...
uncertainty gradually increases. When a landmark comes into range, the extra measurement
constrains the robot position and the predicted standard deviation drops. The empirical
mean absolute error also decreases or grows more slowly in those regions.

In the results obtained here, the Kalman filter also outperforms the corresponding batch
least-squares result. Comparing Deliverable 4 with Deliverable 2, the landmark-aware
...
This result differs from least squares because the Kalman filter is incremental, while
batch least squares solves for the entire trajectory at once. In batch least squares,
all measurements over the whole run are used simultaneously, so later measurements can
influence earlier estimated poses. In contrast, the Kalman filter only uses the
information available up to the current time step and does not retrospectively smooth
the entire trajectory. In spite of that, the Kalman filter produced the lower error in
these runs, while also providing a covariance-based uncertainty estimate that the
least-squares plots do not directly provide.

\section{Deliverable 5: Batch Least Squares with Back-and-Forth Loop Closures}
For Deliverable 5, the batch least-squares formulation from Deliverable 2 was extended
...
Yes, the re-observation of landmarks has a meaningful impact on the robot's ability to
...
\section{Conclusion}
Across all five deliverables, the main pattern is consistent: odometry alone leads to
steadily increasing localization error, while landmark observations reduce drift by
anchoring the robot to fixed features in the hallway. In the results obtained for this
assignment, the Kalman filter outperformed the batch least-squares approach overall,
with the landmark-aware Kalman filter in Deliverable 4 producing the lowest sustained
error among the one-way hallway traversals.

The back-and-forth batch least-squares case in Deliverable 5 still demonstrates an
...
\section{Future Work}
If this simulation were extended beyond the assignment requirements, several next steps
...
the incremental setting. Second, the batch least-squares implementations in Deliverables
1, 2, and 5 could be refactored for readability, since the Kalman filter implementations
...
verified, and submitted by the student.
